...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, hidden1_size=512, hidden2_size=256, 
        ...
        # 第一层 - 使用权重规范化而非批归一化
        self.layer_1 = nn.utils.weight_norm(nn.Linear(state_dim, hidden1_size))
        self.dropout1 = nn.Dropout(dropout_rate)
        
        # 第二层 - 使用权重规范化
        self.layer_2 = nn.utils.weight_norm(nn.Linear(hidden1_size, hidden2_size))
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden1_size=512, hidden2_size=256, dropout_rate=0.1):
        ...
        # 第一个共享层 - 使用权重规范化
        self.shared_layer1 = nn.utils.weight_norm(nn.Linear(state_dim + action_dim, hidden1_size))
        self.dropout1 = nn.Dropout(dropout_rate)
        
        # 第二个共享层 - 使用权重规范化
        self.shared_layer2 = nn.utils.weight_norm(nn.Linear(hidden1_size, hidden2_size))
        self.dropout2 = nn.Dropout(dropout_rate)
        
        # Q1网络
        self.Q1_layer = nn.utils.weight_norm(nn.Linear(hidden2_size, hidden2_size // 2))
        self.Q1_head = nn.Linear(hidden2_size // 2, 1)
        
        # Q2网络
        self.Q2_layer = nn.utils.weight_norm(nn.Linear(hidden2_size, hidden2_size // 2))
