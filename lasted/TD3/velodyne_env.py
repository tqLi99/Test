...
class GazeboEnv:
    ...
    def potential_function(self, x, y):
        distance = np.linalg.norm([x - self.goal_x, y - self.goal_y])
        return -distance  # 负距离作为势能
    ...
    def step(self, action):
        ...
        # 计算当前状态的势能
        current_potential = self.potential_function(self.odom_x, self.odom_y)
        ...
        # 计算新状态的势能
        new_potential = self.potential_function(self.odom_x, self.odom_y)
        ...
        # 计算原始奖励
        original_reward = self.get_original_reward(target, collision, action, min_laser)
        
        # 计算势能差
        potential_diff = self.gamma * new_potential - current_potential
        
        # 组合奖励: R'(s,a,s') = R(s,a,s') + γΦ(s') - Φ(s)
        shaped_reward = original_reward + potential_diff
        
        return state, shaped_reward, done, target
